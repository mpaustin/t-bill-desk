    ...
    <main className="min-h-screen bg-ink">
      <div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-10">
        ...
        <section className="grid gap-6 py-8 lg:grid-cols-[1fr_340px]">
          ...
          <aside className="flex flex-col rounded-2xl border border-line bg-panel p-5 sm:p-6"><div className="mb-6"><p className="font-mono text-xs uppercase tracking-[0.2em] text-forest">New order</p><h2 className="mt-2 text-xl font-semibold text-white">Build a position</h2></div>
            <form onSubmit={requestOrderConfirmation} className="flex flex-1 flex-col">
              <div className="space-y-5">
                <label className="block"><span className="mb-2 block text-xs font-medium uppercase tracking-wider text-slate-400">Maturity term</span><select value={term} onChange={(e) => setTerm(e.target.value)} className="w-full rounded-xl border border-line bg-ink px-3 py-3 text-sm text-white outline-none transition focus:border-forest">{curve?.points.map((point) => <option key={point.term} value={point.term}>{point.term} Treasury · {point.yield.toFixed(2)}%</option>)}</select></label>
                <label className="block" htmlFor="order-amount"><span className="mb-2 block text-xs font-medium uppercase tracking-wider text-slate-400">Order amount</span><div className="relative"><span className="absolute left-3 top-3 text-slate-500">$</span><input id="order-amount" aria-label="Order amount" value={amount} onChange={(e) => setAmount(formatAmountInput(e.target.value))} inputMode="numeric" required aria-invalid={amount.length > 0 && !hasValidAmount} aria-describedby="order-amount-help" className="w-full rounded-xl border border-line bg-ink py-3 pl-8 pr-3 text-sm text-white outline-none transition focus:border-forest" placeholder="10,000" />{amount.length > 0 && !hasValidAmount && <p id="order-amount-help" className="mt-2 text-xs text-red-300">Minimum order amount is $100.</p>}</div></label>
                <div className="flex min-h-[150px] flex-col justify-center gap-5 rounded-xl border border-line bg-ink/70 p-5"><div className="flex items-center justify-between text-sm text-slate-400"><span>Indicative yield</span><span className="font-mono text-lg text-forest">{selectedPoint ? `${selectedPoint.yield.toFixed(2)}%` : "—"}</span></div><div className="flex items-center justify-between text-sm text-slate-400"><span>Estimated annual interest</span><span className="font-mono text-lg text-slate-200">{selectedPoint && amount ? currencyWithCents.format(numericAmount * selectedPoint.yield / 100) : "—"}</span></div></div>
              </div>
              <div className="mt-auto space-y-3 pt-5"><button disabled={submitting || !curve || !hasValidAmount} className="w-full rounded-xl bg-forest px-4 py-3 text-sm font-semibold text-ink transition hover:bg-forest/90 disabled:cursor-not-allowed disabled:opacity-50">{submitting ? "Submitting…" : "Submit order"}</button>{message && <p className={`text-center text-xs ${message.includes("Unable") || message.includes("valid") || message.includes("Minimum") ? "text-red-300" : "text-forest"}`}>{message}</p>}</div>
            </form>
          </aside>
        </section>
