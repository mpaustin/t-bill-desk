    ...
    <main className="min-h-screen bg-ink">
      <div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-10">
        <header className="flex items-center justify-between border-b border-line pb-6">
          ...
          <div className="hidden rounded-full border border-line px-3 py-1.5 text-xs text-slate-400 sm:block"><span className="mr-2 inline-block h-2 w-2 rounded-full bg-forest" />Paper trading mode</div>
        </header>

        <section className="grid gap-6 py-8 lg:grid-cols-[1fr_340px]">
          <div>
            <div className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-semibold tracking-tight text-white sm:text-4xl">Market Snapshot</h1></div><div className="text-right text-xs text-slate-500">{curve ? <>As of <span className="font-mono text-slate-300">{curve.asOf}</span><br />{curve.source === "sample" ? "Demo fallback data" : "U.S. Treasury feed"}</> : "Loading market data…"}</div></div>
          ...
          <aside className="rounded-2xl border border-line bg-panel p-5 sm:p-6"><div className="mb-6"><p className="font-mono text-xs uppercase tracking-[0.2em] text-forest">New order</p><h2 className="mt-2 text-xl font-semibold text-white">Build a position</h2><p className="mt-2 text-sm leading-5 text-slate-400">Choose a term and notional amount. This creates a paper order for review.</p></div>
